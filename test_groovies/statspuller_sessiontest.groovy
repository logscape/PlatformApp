...
//sql.eachRow("select DateADD(mi, -34800, DateADD(mi, -5, Current_TimeStamp))"){row-> println row}





sql.eachRow("select * FROM SESSION_HISTORY WHERE TIME_STAMP > DateADD(mi, -34800, DateADD(mi, -5, Current_TimeStamp)) AND TIME_STAMP < DATEADD(mi,-34795,Current_Timestamp);"){ row -> 
